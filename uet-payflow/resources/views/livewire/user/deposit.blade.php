<main
            class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-5 h-143 flex items-center justify-center">
                @push('title')Deposit Funds @endpush
                @include('alerts')
            <div class="text-white w-md space-y-2">
                <div class="bg-white/5 border border-white/10 p-3 rounded-lg">
                    <p class="text-primary text-2xl font-bold">Deposit Funds</p>
                    <p class="text-xs text-white">Add money to your wallet instantly</p>
                </div>
                <div class="bg-white/5 border border-white/10 p-3 rounded-lg">
                    <div class="flex items-center gap-2">
                        ...
                        <div class="">
                            <p class="text-muted text-xs">Current Balance</p>
                            <p class="text-primary text-xl font-bold">Rs. {{ number_format($available_balance, 2) }}</p>
                        </div>
                    </div>
                </div>
                <form wire:submit.prevent="deposit" class="bg-white/5 border border-white/10 p-3 rounded-lg space-y-3">
                ...
                <button type="button" wire:click="addAmount(10000)" 
                    ...
                    <div class="relative group">
                        <input wire:model="pin" type="password" id="password" name="password"
                        ...
                        <button aria-label="Password toggler" type="button" id="togglePassword"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-300 hover:text-white transition-colors cursor-pointer">
    ...
    <div class="fixed top-0 right-0 w-full h-full flex justify-center items-center z-50">
        <!-- Backdrop -->
        <div class="absolute top-0 right-0 w-full h-full inset-0 bg-black/40 backdrop-blur-sm" wire:click="closeModal"></div>

        <!-- Modal Content -->
        <div class="relative bg-white/5 border border-white/10 rounded-2xl pb-8 px-6 w-sm z-10 shadow-2xl animate-in">
            <!-- Success Icon / Video -->
            <div class="flex justify-center">
                <video class="block mx-auto" width="200px" autoplay src="{{ asset('assets/Done.webm') }}"></video>
            </div>

            <!-- Status Text -->
            <div class="text-center mt-4 space-y-1">
                <p class="text-white text-xl font-bold font-inter">Deposit Successful!</p>
                <p class="text-white text-xs">Your deposit has been processed</p>
            </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-xl p-3 mt-6 space-y-2">
                ...
                <div class="flex justify-between text-xs">
                    ...
                    <span class="text-green-400 font-semibold">+Rs. {{ number_format($lastAmount, 2) }}</span>
                </div>
                <hr class="border-t border-white/5">
                <div class="flex justify-between text-xs">
            ...
            <div class="flex gap-3 mt-6">
                <a  href="{{ route('user.dashboard') }}" wire:navigate
                    class="flex-1 border border-white/10 rounded-xl text-white text-sm text-center bg-white/5 hover:bg-white/10 
